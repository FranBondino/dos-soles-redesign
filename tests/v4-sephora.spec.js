...
test.describe('Version 4: Sephora Vibrant Style Homepage', () => {
  
  test.beforeEach(async ({ page }) => {
    // Navigate directly to the v4-sephora index.html page
    await page.goto('/v4-sephora/index.html');
  });
  ...
  test('should use red (#D4000C) for accents such as cart count background', async ({ page }) => {
    const cartCount = page.locator('.cart-count');
    await expect(cartCount).toBeVisible();
    
    const bgColor = await cartCount.evaluate((el) => {
      return window.getComputedStyle(el).backgroundColor;
    });
    expect(bgColor).toBe('rgb(212, 0, 12)'); // #D4000C
  });
